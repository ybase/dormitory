...
<html>
  <head>
	...
	<script type="text/javascript">
		function selectAll() {
			var sends =document.getElementsByName("send");
			...
			for(var i = 0;i<sends.length;i++) {
				if(selectAll.checked == true) {
					...
					sends[i].checked = false;
				}
			}
		}
		
		function sendOrder(pageOffset,pageSize) {
			var sends = document.getElementsByName("send");
			var count = 0;
			...
			oform.action = "adminSendOrderServlet?orderIds="+orders+"&pageOffset="+pageOffset+"&pageSize="+pageSize;
			oform.submit();
		}
	</script>
  </head>
      ...
      <tr>
    	...
    	<c:forEach var="order" items="${ orderList}">
    		<tr>
    			...
    			<td bgcolor="#FFFFFF" class="STYLE2"><input type="checkbox" name="send" value="${order.orderId }"></td>
        ...
        <td background="Admin/images/tab_21.gif"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
           <td>
    		...
    		<input type="button" value="发货" onclick="sendOrder(${orderPager.pageOffset},${orderPager.pageSize})">
    		</td>
